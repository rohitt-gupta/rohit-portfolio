...
function loadSettings(): { font: FontOption; color: ColorOption } {
  if (typeof window === "undefined") return { font: "schibsted", color: "regular" };
  try {
    ...
    if (raw) {
      ...
      const font =
        parsed.font && FONTS.some((f) => f.id === parsed.font) ? parsed.font : "schibsted";
      return { font, color };
    }
  } catch {}
...
export const Settings = () => {
  ...
  // Preferences live in localStorage, which the server can't read. We deliberately render
  // the defaults first and reconcile after hydration, so this setState-in-effect is intended.
  useEffect(() => {
  ...
  return (
    <div ref={containerRef} className="fixed top-4 right-4 z-50 flex flex-col items-end">
